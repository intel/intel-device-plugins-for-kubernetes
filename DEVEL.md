...
Errors generated within the framework and plugins are instantiated with the `New()` function of the
standard [errors package](https://pkg.go.dev/errors) or with `fmt.Errorf()`:

```golang
    return errors.New("error message")
```

Errors generated from outside the plugins and framework are either returned as is or wrapped with
additional context using the `%w` verb of `fmt.Errorf()` so that the original error remains
available to `errors.Is()`/`errors.As()`:

```golang
    return fmt.Errorf("some additional error message: %w", err)
```

These errors are then logged using a default value format like:
