...
func TestGetDeviceHealth(t *testing.T) {
	for _, tc := range tcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			h1, h2, err := n.GetDeviceHealth("0000:00:00.1")

			if tc.fail == NoError && err != nil {
				t.Error("GetDeviceHealth returned an error:", err)
			}

			if tc.fail == NoError && (!h1 || !h2) {
				t.Error("Call to device health returned unhealthy", h1, h2, tc.fail)
			}
