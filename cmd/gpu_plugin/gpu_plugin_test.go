...
func (m *mockL0Service) GetDeviceHealth(bdfAddress string) (bool, bool, error) {
	if m.fail {
		return false, false, errors.Errorf("error, error")
	}

	return m.healthy, true, nil
}
...
func createBypathTestFiles(t *testing.T, card, root, linkFile string, bypathFiles []string) (string, string) {
	...
	if linkFile != "" {
		...
		if err := os.WriteFile(devPath, []byte{0}, 0600); err != nil {
	...
	if len(bypathFiles) > 0 {
		...
		for _, f := range bypathFiles {
			if err := os.WriteFile(path.Join(byPath, f), []byte{1}, 0600); err != nil {
