...
func main() {
	...
	if _, err := os.Stat(vfioDevDirectory); os.IsNotExist(err) {
		klog.Fatal("VFIO is not supported on this system, /dev/vfio does not exist")
	}
	...
	for _, dpath := range pciDevices {
		...
		driverName := pluginutils.DeviceDriverName(dpath, "unknown")
		if driverName == vfioDriverName {
			continue
		}
