...
ENV CGO_LDFLAGS="-fstack-protector-strong -Wl,-O1,--sort-common,--as-needed,-z,relro,-z,now,-z,noexecstack,-z,defs"
ENV CGOFLAGS="-trimpath -mod=readonly -gcflags=\"all=-spectre=all -N -l\" -asmflags=\"all=-spectre=all\" -buildmode=pie -ldflags=\"all=-linkmode=external -s -w\""
ARG GOLICENSES_VERSION
ARG CMD
ARG ROCKYLINUX
ARG CGO_VERSION=1.22
RUN mkdir /runtime
...
WORKDIR ${DIR}
COPY . .
RUN (export PATH=$PATH:/usr/local/go/bin/ && cd cmd/${CMD}; GO111MODULE=on CGOFLAGS="${CGOFLAGS}" CGO_LDFLAGS="${CGO_LDFLAGS}" CGO_CFLAGS="${CGO_CFLAGS}" CGO_ENABLED=1 go install)
